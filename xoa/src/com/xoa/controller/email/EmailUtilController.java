...
public class EmailUtilController {
	...
	String insertEmailBody(
			...
			@RequestParam(value = "sendFlag") String sendFlag,
		...
		try {
			if(sendFlag.equals("0")){
				...
				emailService.sendEmail(this.returnObj(fromId, toId2, copyToId, subject, content, attachmentName, sendTime, sendFlag, secretToId, attachmentId, smsRemind, important, size, fromWebmailId, fromWebmail, toWebmail, compressContent, webmailContent, webmailFlag, recvFromName, recvFrom, recvToId, recvTo, isWebmail, isWf, keyword, secretLevel, auditMan, auditRemark, copyToWebmail, secretToWebmail, praise)
						, new Email());
			}
			return JSON.toJSONStringWithDateFormat(new ToJson<EmailBody>(0,
					"ok"), "yyyy-MM-dd HH:mm:ss");
		} catch (Exception e) {
			loger.debug("sendMail:" + e);
			return JSON.toJSONStringWithDateFormat(new ToJson<EmailBody>(1,
					"errorEmail"), "yyyy-MM-dd HH:mm:ss");
		}
	}
	...
	public  EmailBody returnObj(String fromId, String toId2, String copyToId,
			 String subject,String content,String attachmentName,
			 String sendTime,String sendFlag,String secretToId,
