...
(function($){
    ...
    $(document).ready(function($){
        ...
        ctroltime = window.setTimeout("sms_mon()", 3000);
...
function email_mon()
{
   jQuery.get("../general/status_bar/email_mon.php", {}, function(data){
      if(data == "1")
         $("new_letter").innerHTML="<a href='#' onclick='javascript:show_email();' title='" + new_email_title + "'><img src='" + static_server + "/static/images/email_close.gif' border='0' width='16' height='16' align='absMiddle'></a>&nbsp;";
      else
...
function sms_mon()
{
   ...
   jQuery.get("../attachment/new_sms/" + cur_uid + ".sms", {now: new Date().getTime()}, function(data){
      sms_mon_flag = data;
        ...
        success: function(rows){
            ...
            if(rows.length > 0)
            {
                for(var i=0; i<rows.length; i++)
                {
                    ...
                    if(rows[i].type == '0')
                    {
                        url = '/general/calendar/arrange/cal_note.php?CAL_ID=' + rows[i].cal_id;
                    }
                    else if(rows[i].type == '1')
                    {
                        url = '/general/calendar/affair/note.php?AFF_ID=' + rows[i].cal_id;
                    }
                    else if(rows[i].type == '2')
                    {
                        url = '/general/calendar/task/note.php?TASK_ID=' + rows[i].cal_id;
                    }
